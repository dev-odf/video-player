// player.js - Step 2: Theme-Adaptive Icons

document.addEventListener("DOMContentLoaded", () => {
  // 1. Detect the Host Website's Theme Color
  let themeColor = '#888888'; // Default grey fallback
  const metaTheme = document.querySelector('meta[name="theme-color"]');
  if (metaTheme && metaTheme.content) {
    themeColor = metaTheme.content;
  }

  // 2. Inject our custom CSS with the dynamic theme variable
  const style = document.createElement('style');
  style.innerHTML = `
    :root {
      /* Store the detected theme color in a CSS variable */
      --vaas-theme: ${themeColor};
    }
    ...
    /* On hover, use the detected site theme (or the grey fallback) */
  ...
  // 4. Find all places the developer wants a player
